defmodule Day08 do
  ## counts is a tuple of two ints, {with_special_chars, without_special_chars}

  # base case
  defp count("", counts), do: counts

  # quote character, unescaped
  defp count("\"" <> rest, {w, wo}), do: count(rest, {w+1, wo})

  # \xNN sequence
  defp count("\\x" <> rest, {w, wo}), do: count(String.slice(rest, 2..-1), {w+4, wo+1})

  # any character, backslashed
  defp count("\\" <> rest, {w, wo}), do: count(String.slice(rest, 1..-1), {w+2, wo+1})

  # any other character
  defp count(str, {w, wo}), do: count(String.slice(str, 1..-1), {w+1, wo+1})


  def count(str), do: count(str, {0,0})

  def parse_and_count(string_block) do
    string_block
    |> String.split("\n")
    |> Enum.map(&count/1)
    |> Enum.reduce({0, 0}, fn ({w1, wo1}, {w2, wo2}) -> {w1+w2, wo1+wo2} end)
  end

  def difference({w, wo}), do: w - wo
end
