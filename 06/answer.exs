defmodule Day06 do
  def toggle(map, coordinate) do
    prev = Map.get(map, coordinate) || 0
    new = if prev == 0, do: 1, else: 0
    Map.put(map, coordinate, new)
  end

  def turn_on(map, coordinate) do
    Map.put(map, coordinate, 1)
  end

  def turn_off(map, coordinate) do
    Map.put(map, coordinate, 0)
  end

  ## "0,0 through 2,2" becomes { {0,0}, {2,2} }
  def get_coordinates_from_range(str) do
    str
    |> String.split(" through ")
    |> Enum.map(fn (c) -> c |> String.split(",") |> Enum.map(&String.to_integer/1) |> List.to_tuple end)
    |> List.to_tuple
  end

  def perform("turn on " <> range, map), do: perform_function(range, map, &turn_on/2)
  def perform("turn off " <> range, map), do: perform_function(range, map, &turn_off/2)
  def perform("toggle " <> range, map), do: perform_function(range, map, &toggle/2)
  def perform(_, map), do: map

  def perform_function(range, map, function) do
    {{x1,y1}, {x2,y2}} = get_coordinates_from_range(range)
    coords = for x <- x1..x2, y <- y1..y2, do: {x,y}
    coords |> Enum.reduce(map, fn (c, m) -> function.(m, c) end)
  end
end
...
|> Enum.reduce(%{}, fn (instruction, map) -> Day06.perform(instruction, map) end)
|> Map.to_list
